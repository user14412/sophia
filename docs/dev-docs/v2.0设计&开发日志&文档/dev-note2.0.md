...
- [x] 工具类化(utils)：将原先的节点逻辑 定死为一个第一性原理：节点是工作流中的一个步骤（不是传统的tools工具），调用工具类（utils）
  - [x] 工具类：将原来散落在节点内的逻辑模块化为工具类、工具函数
- [x] 接口结构化：设计接口的统一数据结构，编写可查的接口文档
  - [x] unix哲学，统一通过高度抽象的状态（json）流转
  - [x] 定义 Agent 的 State（状态）：typedDict规定数据格式 | None
  - [x] 定义 AI 的 输出格式（Schema）：pydantic
- [x] 分层架构设计：分为知识层、内容生成层、表现层
...
- [x] 抽象策略
  ...
  - [x] 模块（阶段）
    ...
    - [x] 模块内部抽象用传统的类和方法实现
    - [x] 模块内流程间传递的数据结构（Unix哲学）单独定义，一般是VideoState的一个字段，本身也是一个TypeDict
- [x] 配音阶段 Voice
- [x] 配图阶段 Image
- [x] 内容层
  - [x] 内容层的抽象策略和视图层不同，内容层属于是节点之间的连接关系复杂，但节点内部实现逻辑简单，视图层是反着来的。
...
- [x] 配音阶段 Voice
  - [x] ChatTTS-特性实验
    - [x] 尽量一个人说一句话的时候，不要用短句号，短句号会被识别为一个单独的chunk，导致和后面的同一个人说的话音色有细微差别，放一起特别明显。
      - [x] 问题：
        - [x] chunk切分策略过于粗糙，导致**断句太短**，说话断断续续，不够连贯。前一句话说完调子还没下去就戛然而止。断句之间的衔接唐突，但这也与断句太短有关。正则表达式的切法太粗糙。怎么自动根据**【语义、情感】的连贯性+【长度限制】**，灵活切分出一条条chunk？
          ...
          - [x] **策略 ：拥抱大模型，做语义级 Chunk 切分** 既然你已经在构建基于 LLM 的 Agent，放弃正则，让 LLM 在上游（或者在 Parser 节点内部）帮你做**“适合配音的呼吸句切分”**。你可以给 LLM 传入原始脚本，并要求它输出 JSON 格式的 Chunk 列表。
            - *Prompt 思路*：“你是一个专业的配音导演。请将以下文案切分为适合一口气读完的短句。要求：1. 保持语义和情感的绝对完整；2. 根据标点符号、转折词和人类呼吸节奏切分；3. 每段长度控制在 20-50 字。”
              - [x] 效果显著提升
        - [x] ChatTTS这里面的音色不好听，非常模糊沙哑，不知道是不是随机种子问题。这样的声音完全无法发布。如果想让TTS质量高一点，声音清晰、听着舒服，能不能直接换付费接口，或者Sovits？
          - [ ] ChatTTS的训练数据包含了大量低质量的播客和网络通话
      - [ ] 解决
        - [x] 或可以直接给LLM做chunk切分，让它自动根据**【语义、情感】的连贯性+【长度限制】**，灵活切分出一条条chunk？
        - [x] 如果script直接生成的就是结构化播客，其实不用做chunk。因为每句话一般不会超过100字。script生成的时候就告诉LLM每句话不超过50字。chunk的时候如果还是发现超过50字直接暴力切分 / 调用原来的chunk策略。
  - [x] GPT-Sovits
    - [x] go-api.bat运行服务
    - [x] 关于底噪和气音：1分钟微调是“降维打击”
      - [x] 海森
        - [ ] sovits_weights: `C:\UserData\models\GPT-SoVITS-v2pro-20250604-nvidia50\SoVITS_weights_v2Pro\haisen_voice_e8_s352.pth`
        - [ ] gpt_weights: 
      - [x] 纳西妲
        - [ ] sovits_weights: 
        - [ ] gpt_weights: 
      - [ ] 已经很好了，但是感觉还有点尾音戛然而止的瑕疵，感觉可以通过一些数学技巧让尾音拖长0.2秒逐渐降低？
      - [ ] 胡桃和海哥这种声音清脆的，天生就比钟离和纳西妲这种气音多的好训
    - [ ] 关于句子衔接与呼吸感：工程化补救策略
      - [ ] 方案 A：在代码层注入“数学静音”（极其有效且精准）
      - [ ] 方案 B：通过标点符号“骗”出尾音
- [x] LLM结构化输出`with_structured_output`
  - [x] 传入的顶层对象必须是一个普通的类或字典，不能是List
...
- 在引号前面加一个英文字母 `r`，意思是“Raw string”（原始字符串），告诉 Python：“忽略里面的所有反斜杠转义，我写什么就是什么”。
...
- 测试框架：pytest 
  ...
  - 不是“设计了一套复杂系统”，而是“把所有历史决策暴露成参数”
  
  - 必须耐下性子写文档了，不写文档后面考古式开发有你受的。这是刚需


## bug to fix

- [x] 文生图 429 to many request，容易被限流。通过指数退避重试机制解决。
...
纳西妲 x 钟离，以赵林《西方哲学史讲演录》为大纲，基于专业知识库，生成100集高质量对谈播客。适合时间紧张的人，不用看书就能以通俗的方式入门哲学。还可以进一步引入高质量曲库，适时插入符合语义和情感的高质量bgm，让听众身临其境。
